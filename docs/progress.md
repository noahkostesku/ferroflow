...
Last updated: 2026-04-29 (Week 8 Session 1)
...
- **2026-04-27:** Week 7 Session 3. GPU benchmark on Narval A100 (job 59957529). matmul-parallel 16×8 branches, 2048×2048: CPU 8w = 25 ops/s (5,101ms), A100 = 62 ops/s (2,049ms), 2.5× speedup. 4096×4096 GPU: 17 ops/s (3,689ms). Bottleneck is per-op CPU↔GPU tensor transfer. Issue #8 closed.
- **2026-04-29:** Week 8 Session 1. Multi-cluster benchmark complete (issue #9). Nibi H100-80GB-HBM3: 52 ops/s / 2,473ms at 2048×2048 matmul-parallel (128 ops, 8 workers, avg_batch=12.6). Narval A100 comparable run: 47 ops/s / 2,727ms (avg_batch=12.4). H100 advantage: +10.6% throughput, -9.3% wall time. CPU baselines equivalent (Narval=25 ops/s, Nibi=26 ops/s), confirming gain is GPU-specific. README updated with multi-cluster section and new results table. Issue #9 closed.
- **2026-05-06:** P2P vs Coordinator scaling study (job 60529184, branch p2p-stealing). xlarge-wide DAG (2/4/8 nodes): P2P shows 4-5× lower throughput than coordinator WS (e.g. 1,861 vs 7,428 ops/s at 2 nodes) with high steal rate (130-254/s) — P2P protocol overhead dominates on balanced workloads. Separate mpirun validation on imbalanced DAG: P2P achieves 2× over coordinator (403 vs 202 ops/s), confirming P2P is effective when genuine imbalance exists. **Blocker:** xlarge-transformer P2P deadlocked; fix in progress. Imbalanced DAG Narval run not reached. Next step: adaptive scheduler selection (P2P on imbalanced, coordinator on balanced). Results logged in `docs/benchmarks.md`.
